...
| 12 | [`s2-quotation-service:createQuotation` (~line 116) + `createGroupQuotation` (~line 361)](../back_end/src/services/domain/s2-quotation-service.ts) | **OVER-charge on per-night seals.** `roomCount = Math.max(1, sealed.distinctRoomIds.length \|\| entry.numberOfRooms)` counts *distinct* rooms across the whole stay, but a mid-stay room change occupies only `numberOfRooms` rooms **per night**. The backend itself validates every night has exactly `numberOfRooms` rooms, so room-nights are always `numberOfRooms × nights` — never `distinctRooms × nights`. The comment above the line ("total room-nights = distinctRooms × nights") is the mistaken premise. Invisible for "same rooms every night" (distinct == numberOfRooms) — only bites per-night-varying seals, which is why the 2026-07-13 sweep missed it. | **Open** — found 2026-07-16 via smoke test. Repro: 2-room / 3-night booking, seal night1={205,206} night2={302,303} night3={205,307} (5 distinct rooms) → quotation returned `roomCount:5, subTotal:39375` when it should be `roomCount:2, subTotal:15750` (2.5× overcharge). Fix: derive `roomsPerNight = max(perNight[].roomIds.length) ?? distinctRoomIds.length` and use that. Deferred per user request (backend fix to come later). | — |
