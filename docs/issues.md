...
**🔴 C1 — Authentication is fully spoofable; actor level is self-asserted via HTTP header (P22, P3, P4)**
[auth.ts:11-26](Backend_Revamp-main/back_end/src/middleware/auth.ts:11) reads `x-actor-level` straight from the request and trusts it; [api-router.ts:53](Backend_Revamp-main/back_end/src/routes/api-router.ts:53) mounts this globally. The login path _issues_ a signed JWT, but **`jwt.verify` is never called anywhere in `src/` (0 matches)**. Any client sending `x-actor-level: L4` gets full GM/admin authority over every endpoint. This defeats _every_ downstream authority gate (waivers, write-offs, FOC, overbooking). The §10.2 note framed this as "controller-vs-service inconsistency"; the real state is **no enforced authentication at all**. This is the single most important finding.
→ Compounded by [session-service.ts:7](Backend_Revamp-main/back_end/src/services/infrastructure/session-service.ts:7): `JWT_SECRET ?? "dev-jwt-secret"` hardcoded fallback, and [require-gm-role.ts](Backend_Revamp-main/back_end/src/lib/admin/require-gm-role.ts) (the service-layer GM re-check) is **dead code with zero callers**.
