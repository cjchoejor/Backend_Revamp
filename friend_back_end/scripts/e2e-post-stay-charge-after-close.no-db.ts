...
const L1: Actor = { id: "e2e-fd-7", level: "L1" };
const L2: Actor = { id: "e2e-fom-7", level: "L2" };
...
async function main() {
  ...
  // S1
  const inq = await runStep("S1 create inquiry", { method: "POST", path: "/inquiries", actor: L1, body: { guestProfileId: guest.id, sourceChannel: "WALK_IN", notes: "e2e post-stay charge after close" } });
  ...
  // complete pre-arrival tasks + assign room + accept/fulfil H1 + S5->S6
  ...
  // S6: H2 + identity + complete check-in (S7) which auto-creates H3
  ...
  // S9: dispatch draft invoices then close
  ...
  // Post-stay charge after closure (same route /folios/:id/charges but S9 mode requires L2 and isPostStay=true)
  const postedAt = new Date(Date.now() + 2 * 86400_000).toISOString();
  expectStatus(
    "S9 post-stay charge after close",
    await runStep("S9 post-stay charge after close", {
      method: "POST",
      path: `/folios/${folioId}/charges`,
      actor: L2,
      body: { entryId, lineType: "OTHER", description: "Minibar after checkout", amount: 30, currency: "BTN", postedAt, isPostStay: true } as any,
    }),
    200,
  );
  ...
  const outMd = path.join(outDir, "E2E-post-stay-charge-after-close.no-db.md");
...
main()
  .then(() => process.exit(0))
  .catch((e) => {
    console.error("E2E post-stay charge flow: FAILED", e);
    process.exit(1);
  });
