...
import { runPostCheckoutInspectionWorker } from "../src/workers/w9-post-checkout-inspection-worker.js";
...
const L1: Actor = { id: "e2e-fd-9", level: "L1" };
const L2: Actor = { id: "e2e-fom-9", level: "L2" };
...
async function main() {
  ...
  // S1
  const inq = await runStep("S1 create inquiry", { method: "POST", path: "/inquiries", actor: L1, body: { guestProfileId: guest.id, sourceChannel: "WALK_IN", notes: "e2e deferred inspection lapse" } });
  ...
  // complete tasks + assign room + accept/fulfil H1 + progress to S6
  ...
  // S6: H2 + identity + complete check-in to S7
  ...
  // S7: night audit + initiate H4 + progress to S8
  ...
  // S8: key return + DEFERRED inspection + fulfil H4 + settle CASH
  ...
  expectStatus("S8 deferred inspection", await runStep("S8 deferred inspection", { method: "POST", path: `/entries/${entryId}/room-inspection`, actor: L1, body: { isDeferred: true, deficientFlagStatus: "NOT_APPLICABLE", damageFound: false } as any }), 200);
  ...
  expectStatus("S8 settle CASH", await runStep("S8 settle CASH", { method: "POST", path: `/folios/${folioId}/settle`, actor: L1, body: { settlementMethod: "CASH", paymentVerificationRef: `CASH-${Date.now()}`, billingModelConfirmation: "GUEST_PAY" } as any }), 200);

  // Fire W9 worker immediately to create WINDOW_EXPIRED trace to satisfy S9 closure inspection gate.
  await runWorkerStep("W9 post-checkout inspection window expired (manual trigger)", "POST_CHECKOUT_INSPECTION_W9", async () =>
    runPostCheckoutInspectionWorker(prisma, { entryId }),
  );
  ...
  for (const i of (inv.json as any[]) ?? []) {
    if (i?.state === "DRAFT") {
      expectStatus(`S9 dispatch invoice ${i.id}`, await runStep(`S9 dispatch invoice ${i.id}`, { method: "POST", path: `/invoices/${i.id}/dispatch`, actor: L1, body: { dispatchedTo: "guest@example.com" } }), 200);
    }
  }
  ...
  const outMd = path.join(outDir, "E2E-deferred-inspection-lapse-s1-to-s9.no-db.md");
...
main()
  .then(() => process.exit(0))
  .catch((e) => {
    console.error("E2E deferred inspection lapse flow: FAILED", e);
    process.exit(1);
  });
