...
const L1: Actor = { id: "e2e-fd-5", level: "L1" };
const L2: Actor = { id: "e2e-fom-5", level: "L2" };
...
async function main() {
  ...
  // S1
  const inq = await runStep("S1 create inquiry", { method: "POST", path: "/inquiries", actor: L1, body: { guestProfileId: guest.id, sourceChannel: "WALK_IN", notes: "e2e cancel flow" } });
  ...
  // S3 setup enough to confirm to S4
  ...
  expectStatus("S3 record advance payment", await runStep("S3 record advance payment", { method: "POST", path: `/folios/${folioId}/payments`, actor: L1, body: { entryId, amount: 500, notes: "advance for cancellation scenario" } }), 201);
  expectStatus("S3 reconcile advance payment", await runStep("S3 reconcile advance payment", { method: "POST", path: `/folios/${folioId}/advance-payment/reconcile`, actor: L1, body: { entryId, note: "reconcile for cancellation scenario" } }), 200);
  expectStatus("S3 cancellation disclosure", await runStep("S3 cancellation disclosure", { method: "POST", path: `/entries/${entryId}/disclosures/cancellation`, actor: L1, body: { noShowTreatmentStatement: "Standard cancellation disclosure", disclosedTerms: { windowHours: 24 } } }), 201);
  expectStatus("S3 committed hold", await runStep("S3 committed hold", { method: "POST", path: `/entries/${entryId}/holds/committed`, actor: L1, body: { roomId: firstRoomId, commercialJustification: "Cancellation scenario hold" } }), 201);
  ...
  // Cancel at S5 (L2)
  const cancel = await runStep("S5 cancel entry", { method: "POST", path: `/entries/${entryId}/cancel`, actor: L2, body: {} });
  expectStatus("S5 cancel entry", cancel, 200);

  const outDir = path.resolve(process.cwd(), "..", "Documentation_V2");
  fs.mkdirSync(outDir, { recursive: true });
  const outMd = path.join(outDir, "E2E-cancel-s1-to-s5-test-report.no-db.md");
...
main()
  .then(() => process.exit(0))
  .catch((e) => {
    console.error("E2E cancellation flow: FAILED", e);
    process.exit(1);
  });
