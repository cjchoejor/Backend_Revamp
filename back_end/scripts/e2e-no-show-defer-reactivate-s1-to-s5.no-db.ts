...
const L1: Actor = { id: "e2e-fd-6", level: "L1" };
const L2: Actor = { id: "e2e-fom-6", level: "L2" };
...
async function main() {
  ...
  // S1 -> S4 confirm -> S5 activation (same setup as other e2e flows)
  const inq = await runStep("S1 create inquiry", {
    method: "POST",
    path: "/inquiries",
    actor: L1,
    body: { guestProfileId: guest.id, sourceChannel: "WALK_IN", notes: "e2e no-show defer/reactivate flow" },
  });
  ...
  // Provide advance payment + reconcile so holds/confirmation succeed.
  expectStatus("S3 record advance payment", await runStep("S3 record advance payment", { method: "POST", path: `/folios/${folioId}/payments`, actor: L1, body: { entryId, amount: 500, notes: "advance for defer scenario" } }), 201);
  expectStatus("S3 reconcile advance payment", await runStep("S3 reconcile advance payment", { method: "POST", path: `/folios/${folioId}/advance-payment/reconcile`, actor: L1, body: { entryId, note: "reconcile for defer scenario" } }), 200);
  ...
  expectStatus("S3 committed hold", await runStep("S3 committed hold", { method: "POST", path: `/entries/${entryId}/holds/committed`, actor: L1, body: { roomId: firstRoomId, commercialJustification: "No-show defer scenario hold" } }), 201);
  ...
  // DEFER path: schedules AWAITING_WRITTEN_CONFIRMATION_W5 timer and records trace.
  const defer = await runStep("S5 no-show DEFER", {
    method: "POST",
  ...
  expectStatus("S5 no-show DEFER", defer, 200);

  // REACTIVATE: cancels awaiting confirmation timer and clears cutoff flag so S5 can proceed.
  const reactivate = await runStep("S5 no-show REACTIVATE", {
    method: "POST",
    path: `/entries/${entryId}/no-show`,
    actor: L2,
    body: {
      determinationPath: "REACTIVATE",
      contactAttemptLog: [{ channel: "PHONE", attemptedAt: new Date().toISOString(), outcome: "CONNECTED", response: "Arriving late" }],
      decisionReason: "Guest confirmed arrival",
    },
  });
  expectStatus("S5 no-show REACTIVATE", reactivate, 200);

  const outDir = path.resolve(process.cwd(), "..", "Documentation_V2", "test");
  fs.mkdirSync(outDir, { recursive: true });
  const outMd = path.join(outDir, "E2E-no-show-defer-reactivate-s1-to-s5-test-report.no-db.md");
...
main()
  .then(() => process.exit(0))
  .catch((e) => {
    console.error("E2E no-show defer/reactivate flow: FAILED", e);
    process.exit(1);
  });
