...
export async function selectOption(
  ...
  const room = await prisma.room.findUnique({ where: { id: input.roomId }, include: { deficientConditionRecords: true } });
  ...
  return prisma.$transaction(async (tx) => {
    const updated = await tx.availabilityConfiguration.update({
      where: { id: configId },
      data: {
        optionSelected: { roomId: input.roomId, isDeficient },
    ...
    await tx.traceEvent.create({
      data: {
        ...
        timestamp: new Date(),
        entryId: cfg.entryId,
        payload: { configId, roomId: input.roomId },
        createdBy: actorId,
      },
    });
    return updated;
