...
export function BackendView({ entryId }: { entryId: string }) {
  ...
  if (entryQuery.isError || !entry) {
    return (
      <div className="view">
        ...
        <Link className="btn btn-ghost btn-sm" href="/bookings" style={{ marginTop: 12 }}>
          Back to bookings
        </Link>
      </div>
    );
  ...
  return (
    <section className="view bv-root">
      <button className="ws-back" onClick={() => router.push(`/bookings/${entryId}`)} style={{ marginBottom: 12 }}>
        <ChevronLeft />
        Back to workspace
      </button>
