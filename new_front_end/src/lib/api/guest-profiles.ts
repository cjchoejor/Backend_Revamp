...
/** One guest's record — `GET /api/guest-profiles/:id`. */
export async function getGuestProfile(session: Session, id: string) {
  return apiRequest<GuestProfileSummary>(`/api/guest-profiles/${encodeURIComponent(id)}`, { session });
}
