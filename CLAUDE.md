...
|---|---|---|
| `main` | `back_end/` + user's testing `my_front_end/` | Daily dev branch — the user's testing UI + backend, untouched by friend's code. |
| `integration-prod-frontend` | `back_end/` + user's `my_front_end/` + `friend_back_end/` + `front_end/` | Integration branch. Holds the friend's production frontend + a snapshot of the friend's backend for reference. User's backend is the source of truth on both branches. |

The friend pushes to his branch `UI-experiment2` on the same repo. To pull his latest frontend into the integration branch, see **Working conventions → Pulling friend's latest from UI-experiment2**.

**Folders renamed 2026-07-31.** The testing UI moved `front_end/` → `my_front_end/` on **both** branches, and the friend's UI moved `friend_front_end/` → `front_end/` on the integration branch only. Reason: his UI lives at `front_end/` on his branch, and git merges by path — so while the names disagreed, pulling his work could only ever be a destructive folder copy, and merging his branch would have poured his UI into the testing UI's folder. The rename had to happen on `main` too, otherwise the next `main → integration` merge would pour main's testing UI straight into the friend's UI folder: same name, two different meanings.
...
The friend's frontend (at `front_end/` on the integration branch) is wired to run against **user's backend** (at `back_end/`). Both were forked from the same base, so the auth contract, endpoint shapes, and DTO structure are byte-equivalent — **no code changes** were needed on his frontend. The wiring is purely config:
...
- Friend's `apiRequest` client at [`front_end/src/lib/api/client.ts`](front_end/src/lib/api/client.ts) already sends `Authorization: Bearer <jwt>` from the stored session — matches user's JWT auth middleware.
...
git rm -r my_front_end   # ONLY if user decides to retire the testing frontend
...
- **Admin UI**: `/admin/house-tariff` in the friend's frontend ([page.tsx](front_end/src/app/(app)/admin/house-tariff/page.tsx)), nav entry under **Domain 03 · Commercial** between Rate plans and Seasons. Client fns `getHouseTariff` / `listHouseTariffVersions` / `saveHouseTariff` in [`lib/api/admin.ts`](front_end/src/lib/api/admin.ts). The page shows a live "what each plan will charge" table distinguishing explicitly-set prices from ones added up from their meals, plus the version history. Note `front_end` DOES have a full 32-page admin console under `src/app/(app)/admin/` — a glob that doesn't traverse the `(app)` route group will wrongly report none.
...
### Pulling friend's latest from UI-experiment2 (integration branch only)

His current branch is **`UI-experiment2`** (not `UI-experiment` — that is the older one).

**Folder names line up as of the 2026-07-31 rename.** His UI is `front_end/` on his branch and `front_end/` here, so `git merge` can finally do a real three-way merge on it instead of a destructive copy. That is the intended future flow — but it has **not been exercised yet**, and it carries one serious hazard: merging his branch also drags his `back_end/` (124 files, and he makes real backend changes) on top of ours, which is the source of truth. `.gitattributes` marks `back_end/**` as `merge=ours`, but that driver only takes effect if the machine has run `git config merge.ours.driver true` — **verify that before ever merging his branch**, or your backend gets overwritten.

Until that flow is proven, use the copy-and-replace below.

**Copy-and-replace (proven, but destroys local edits inside `front_end/`).** Since the rename, our own work lives in `front_end/` — the house-tariff admin page, the 2-decimal money rule, the renegotiate UI. A blind copy wipes all of it, so take a patch first and replay it after.

```bash
git checkout integration-prod-frontend
git fetch origin UI-experiment2

# 0. Save our own changes to his UI since the last sync. Find the last sync commit with:
#      git log --oneline --grep="Sync friend"
git diff <last-sync-commit>..HEAD -- front_end > ../our-frontend-work.patch

# 1. Temporary parallel checkout of his branch (git allows only ONE branch per folder)
git worktree add ../friend-tmp origin/UI-experiment2

# 2. Wipe so files he DELETED actually disappear here too
rm -rf friend_back_end front_end

# 3. Copy across. NOTE the source names are HIS layout: back_end/ and front_end/.
#    friend_back_end/ is reference-only — we never wire against it.
cp -r ../friend-tmp/back_end ./friend_back_end
cp -r ../friend-tmp/front_end ./front_end

# 4. Throw the worktree away
git worktree remove ../friend-tmp

# 5. Replay our work. --3way gives real conflict markers instead of failing silently.
git apply --3way ../our-frontend-work.patch

# 6. Commit + push
git add -A friend_back_end front_end
git commit -m "Sync friend_back_end + front_end from UI-experiment2"
git push
```
