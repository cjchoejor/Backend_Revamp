...
- **Discount lives with the rates.** `NegotiationDiscountBar` (exported from [room-compositions-table.tsx](front_end/src/components/desk/workspace/room-compositions-table.tsx), rendered by **both** planner modes so it survives a mode switch, and inside `.rct` so it stays on screen in the table's expanded layer) carries discount % + basis. Still **booking-wide**, not per room — the backend folds it into the resolved room rate *before* the compositions are costed, so the strip says outright what that implies: meals and extra beds are never discounted, and a room with its own `negotiatedRoomRate` prices the same discounted or not (it warns when both are set on the same room). No money is computed there — same rule as the rest of the planner.
...
- **The panel stays open after the quote is generated.** It renders in one of two positions: above the send/answer blocks when no quote exists yet (it *is* the step), below them once one does (so Send stays the lead), and closes only on acceptance — where terms bind and the backend refuses to supersede. Its button switches to **"Regenerate quote"** / **"Regenerate quote (new round)"**, which is `supersedeQuotation` with the current compositions + discount: a fresh QUO number, fully re-priced, prior version kept as superseded history.
